...
/** Computes  FFT's of real, single precision data where n is an integral power of 2.
  ...
  * <A name="transformlayout">For RealFloatFFT_Radix2, the correspondence is as follows:
  * <table>
...
public class RealFloatFFT_Radix2 extends RealFloatFFT {
	private static final Logger logger = LoggerFactory.getLogger(RealFloatFFT_Radix2.class);
	private int logn;

	/**
	 * Create an FFT for transforming n points of real, single precision data. n
	...
	public void transform(float data[], int i0, int stride) {
		...
		for (int i = 1; i <= logn; i++) {
			...
			for (b = 0; b < q; b++) {
				float t0_real = data[i0 + stride * b * p]
						+ data[i0 + stride * (b * p + p_1)];
				float t1_real = data[i0 + stride * b * p]
						- data[i0 + stride * (b * p + p_1)];
...

			{
				...
				for (a = 1; a < (p_1) / 2; a++) {
					...
					for (b = 0; b < q; b++) {
						...
						/* t0 = z0 + w * z1 */
						data[i0 + stride * (b * p + a)] = z0_real + w_real
								* z1_real - w_imag * z1_imag;
						data[i0 + stride * (b * p + p - a)] = z0_imag + w_real
								* z1_imag + w_imag * z1_real;
						/* t1 = -(z0 - w * z1) */
						data[i0 + stride * (b * p + p_1 - a)] = z0_real
								- w_real * z1_real + w_imag * z1_imag;
						data[i0 + stride * (b * p + p_1 + a)] = -(z0_imag
								- w_real * z1_imag - w_imag * z1_real);
					}
				}
			}
...

			{
				...
				for (a = 1; a < (p_1) / 2; a++) {
					...
					for (b = 0; b < q; b++) {
						...
						data[i0 + stride * (b * p + p_1 + a)] = (w_real
								* t1_real - w_imag * t1_imag);
						data[i0 + stride * (b * p + p - a)] = (w_real * t1_imag + w_imag
								* t1_real);
					}
				}
			}
