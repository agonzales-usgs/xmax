 ...
 * stored in a 1-dimensional array in Row-Major order. The physical layout in
 * the array data, of the mathematical data d[i,j] is as follows:
 * 
 * <PRE>
 ...
 * href="package-summary.html#wraparound">wrap-around</A> order along each
 * dimension.
 * 
...
public class ComplexFloat2DFFT {
	...
	/**
	 * Return data in wraparound order. rowspan is used to traverse data; the
	 * new array is in packed (rowspan = 2*ncols) format.
	 * 
	 * @see <a href="package-summary.html#wraparound">wraparound format</A>
	 */
	public float[] toWraparoundOrder(float data[], int rowspan) {
		...
		for (int i = 0; i < nrows; i++)
			for (int j = 0; j < ncols; j++) {
				...
				newdata[i * 2 * ncols + 2 * j + 1] = data[i * rowspan + 2 * j
						+ 1];
			}
		return newdata;
	}
