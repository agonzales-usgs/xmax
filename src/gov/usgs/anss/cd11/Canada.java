...
public class Canada {
	...
	private static final int CANCOMP_NOT_20 = 1; /*
	 ...
	 * 
	 * @param b
	 *            The ByteBuffer being used to compress the data
	 * @param data
	 *            The data samples as int[].
	 * @param m
	 *            The number of samples (must be divisible by 20)
	 * @param v0
	...
	public static int canada_compress(ByteBuffer b, int[] data, int m, int v0) {
		...
		// second difference. y[0] is not modified and remains a first
		// difference
		for (int k = m - 1; k > 0; k--)
			y[k] -= y[k - 1];
		// sb.append("Comp ");sb.append(first+","); for(int i=0; i<m; i++)
		// sb.append(y[i]+",");sb.append("->");
		...
		for (int i = 0; i < m; i = i + 4) {
			y0 = y[i];
			if (y0 < 0)
				y0 = (0xffffffff ^ y0); // convert to positive, but one less
										// (abs(y0) -1)
			y1 = y[i + 1];
			if (y1 < 0)
				y1 = (0xffffffff ^ y1);
			...
			if (y3 < 0)
				y3 = (0xffffffff ^ y3);
			y4 = y0 | y1 | y2 | y3; // this masks all of the bits that will be
									// needed as positive values
			if ((y4 & 0x78000000) != 0)
				pm[i / 4] = 32; // need top 4 bits so more than 28 bits needed
								// bits needed
			else if ((y4 & 0x07800000) != 0)
				pm[i / 4] = 28;
			else if ((y4 & 0x00780000) != 0)
				pm[i / 4] = 24;
			else if ((y4 & 0x00060000) != 0)
				pm[i / 4] = 20;
			else if ((y4 & 0x00018000) != 0)
				pm[i / 4] = 18;
			else if ((y4 & 0x00006000) != 0)
				pm[i / 4] = 16;
			else if ((y4 & 0x00001800) != 0)
				pm[i / 4] = 14;
			else if ((y4 & 0x00000600) != 0)
				pm[i / 4] = 12;
			else if ((y4 & 0x00000180) != 0)
				pm[i / 4] = 10;
			else if ((y4 & 0x00000060) != 0)
				pm[i / 4] = 8;
			else if ((y4 & 0x00000018) != 0)
				pm[i / 4] = 6;
			else
				pm[i / 4] = 4;
			// sb.append(pm[i/4]+",");
		...
		int j = m / 10; // offset where data portion starts, first m/10 bytes
						// are keys (1 short for each 20 samples)
		b.position(j);
		...
		for (int i = 0; i < m / 10; i = i + 2) { // for each group of 20 samples
			...
			if (maxbits > 18) { // 4, 8, 12, 16, 20, 24, 28, 32 bits/sample
				for (int k = pmi; k < pmi + 5; k++) {
					pm[k] += pm[k] & 2; // adjust the number of bits to the
										// right range
					j = pack(pm[k], y, k * 4, b, j); // pack 4 samples of width
														// pm[k] into b from y
														// at offset i*4
				}
				// build up the key, note key value is bit (value -4)/4
				short im = (short) (((pm[pmi] - 4 << 10)
						| (pm[pmi + 1] - 4 << 7) | (pm[pmi + 2] - 4 << 4)
						| (pm[pmi + 3] - 4 << 1) | (pm[pmi + 4] - 4 >> 2)) & 0xffff);
				b.position(i);
				...
				for (int k = 0; k < 5; k++) {
					...
					if (b.position() != j)
						if (dbg)
							System.out.println("J and B are out of position");
					pm[pmi + k] -= 4; // convert to key value*2 (the *2 is
										// removed in the shifting)
				}
				short im = (short) (((pm[pmi] << 11) | (pm[pmi + 1] << 8)
						| (pm[pmi + 2] << 5) | (pm[pmi + 3] << 2) | (pm[pmi + 4] >> 1)) & 0xffff);
				b.position(i);
				b.putShort(im);
			}
		}
	 ...
	 * 
	 * @param m
	 *            width of data to pack
	 * @param y
	 *            array with the data samples
	 * @param offset
	 *            offset in y to start (next for samples)
	 * @param b
	 *            The byte buffer where data will be put
	 * @param j
	...
	public static int pack(int m, int[] y, int offset, ByteBuffer b, int j) {
		...
		case 4:
			...
			break;
		case 6:
			ul = ((y0 & 0x3f) << 26) | (y1 & 0x3f) << 20 | (y2 & 0x3f) << 14
					| (y3 & 0x3f) << 8;
			b.putInt(ul);
			j += 3;
			b.position(j);
		...
		case 8:
			...
			break;
		case 10:
			ul = (y0 & 0x3ff) << 22 | (y1 & 0x3ff) << 12 | (y2 & 0x3ff) << 2
					| (y3 & 0x3ff) >> 8;
			b.putInt(ul);
			b.put((byte) (y3 & 0xff));
			j += 5;
	...
	public static void main(String[] args) {
		...
		for (int off = 0; off < offs.length; off++) {
			...
			try {
				...
				logger.error("CanadaException e:", e);
			}
			// Canada.clearSB();
		}
		...
		for (int j = 0; j < 100000000; j++) {
			...
			for (int i = 0; i < 40; i = i + 4) {
				...
				for (int k = i; k < i + 4; k++)
					data[k] = (int) (ran.nextDouble() * offs[ind]);
			}
			int len = Canada.canada_compress(bb, data, 40, 0);
			try {
				Canada.canada_uncompress(buf, out, len, 40, v0);
