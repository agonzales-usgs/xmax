...
/**
 * Simple utility for testing program output. Intercepts System.out to print
 * both to the console and a buffer. From 'Thinking in Java, 3rd ed.' (c) Bruce
 * Eckel 2002
 * 
 * Adapted by D.Ketchum to simultaneously write the SESSION.OUT file and to put
 * output on standard out. Added ring buffer of output which can be checked for
 * errors and then trigger dialog box to send the ring buffer of output by mail
 * to D. Ketchum
 * 
 */
public class TestStream extends PrintStream {
	...
	public TestStream(String className, boolean append) {
		super(System.out, true); // Autoflush
		System.out.println("About to change out/err to " + className.toString()
				+ " with append=" + append);
		System.setOut(this);
		System.setErr(this);
		sb = new StringBuffer(20000);
	...
	/**
	 * when set true, no output goes to console print stream and no dialog boxes
	 * are put up for mailing when an excption is detected
	 * 
	 * @param t
	 *            If true, ture n off console print stream and disable exception
	 *            based mail via dialog
	 */
	...
	/**
	 * when set true, this make the "append" function which keeps the last 10000
	 * characters available will not do dialog boxes if SQLException or
	 * EventQueueExceptoinHander are found. It also disables dialog boxes which
	 * would be checked if noConsole was false.
	 * 
	 * @param t
	 *            if true, set noInteractive mode
	 */
	...
	private synchronized void append(String s) {
		...
		if (s.indexOf("EventQueueExceptionHandler") >= 0) {
			...
			if (s.indexOf("failed to decode") >= 0
					&& s.indexOf("NumberFormatExc") >= 0)
			...
			exceptionOccurred = true;
			if (!ignoreSQLDialog) {
				TimedDialog tm = new TimedDialog(sb, "SQLException " + extra,
						3000);
			}
		} else if (s.indexOf("SeedLinkException") >= 0) {
			exceptionOccurred = true;
			...
			TimedDialog tm = new TimedDialog(sb, "SeedLinkException " + extra,
					3000);
			...
			if (i > 0)
				...
				extra = "";
			TimedDialog tm = new TimedDialog(sb, "RuntimeException " + extra,
					3000);
			...
			TimedDialog tm = new TimedDialog(sb, "DuplicateException" + extra,
					3000);
	...
	public synchronized void write(byte[] buffer, int offset, int length) {
		...
		for (int i = offset; i < offset + length; i++)
			append(String.valueOf(buffer[i]));
		if (!noConsole)
	 ...
	 * displayed. If he user answers yes, the ring buffer of output is sent to
	 * dave
	 */
	...
	class TimedDialog extends Thread {
		...
		public void run() {
			...
			int ans = 0;
			// If this is a interactive application, show a dialog and let the
			// user decide
			try {
				if (!noConsole && !noInteractive)
					ans = JOptionPane
				...
				if (e.toString().indexOf("HeadlessException") >= 0) {
					numberHeadless++;
					Util.prt("Got a HeadlessException in " + Util.getProcess()
							+ " setting noInteractive " + numberHeadless);
					noInteractive = true;
					e.printStackTrace();
				} else {
					e.printStackTrace();
				}
				if (numberHeadless % 100 == 0)
					System.exit(1);
				return;
			} catch (Exception e) {
				...
				if (e.toString().indexOf("HeadlessException") >= 0) {
					numberHeadless++;
					Util.prt("Got a HeadlessException2 in " + Util.getProcess()
							+ " setting noInteractive " + numberHeadless);
					noInteractive = true;
					e.printStackTrace();
				} else {
			...
			if (ans == 0 || ans == JOptionPane.YES_OPTION) {
				...
				if (numberExceptions > 100) {
					if (System.currentTimeMillis() - lastException < 10000) {
						skippedExceptions++;
						Util.prt("Skip Excpt =" + skippedExceptions + " of "
								+ numberExceptions + " " + Util.getNode() + " "
								+ User.getUser() + " " + title + "\n"
								+ Util.getProcess() + " "
								+ System.getProperty("user.home") + " "
								+ System.getProperty("user.dir") + " "
								+ Util.getNode() + " " + Util.ascdate() + " "
								+ Util.asctime() + "\n" + sb.toString());
						return;
					} else
						title = "STORM " + title;
				...
				SimpleSMTPThread.email(
						Util.getProperty("emailTo"),
						"_Exception:" + User.getUser() + " " + title, // subject
						Util.ascdate() + " " + Util.asctime() + " "
								+ User.getUser() + " " + Util.getIDText()
								+ "\n" + sb.toString() + "\n");
			}
		}
	}
