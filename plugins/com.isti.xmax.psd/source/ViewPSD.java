...
public class ViewPSD extends JDialog implements PropertyChangeListener, ChartProgressListener {
	...
	private static final Logger logger = LoggerFactory.getLogger(ViewPSD.class);
			...
			if (value.equals("Close")) {
				...
				for (int i = 0; i < dataset.getSeriesCount() - 2; i++) {
					...
					try {
						...
						try {
							...
							// do nothing
							logger.error("Cant close buffered output stream");
						}
					}
				}
				...
				for (int i = 0; i < dataset.getSeriesCount() - 2; i++) {
					...
					try {
						...
						try {
							...
							// do nothing
							logger.error("Cant close data output stream");
						}
					}
				}
				...
				try {
					...
					try {
						...
						// do nothing
						logger.error("Cant close data stream");
					}
				}
